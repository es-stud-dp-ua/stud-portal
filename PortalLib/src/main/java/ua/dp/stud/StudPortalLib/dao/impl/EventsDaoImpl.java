...
public class EventsDaoImpl extends BaseDaoImpl<Events> implements EventsDao<Events>{

    @Override
    public void deleteEvent(Integer id) {
        Events event = (Events) getSession().createQuery("Select events from Events events Where events.id = :id").setParameter("id", id).uniqueResult();
        ImageImpl image = event.getMainImage();
    ...
    public Events getEventsById(Integer id) {
        Events event = (Events) getSession().get(Events.class, id);
        Hibernate.initialize(event.getAdditionalImages());
        return event;
    }
